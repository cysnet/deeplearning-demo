...
from LetNet5_model_back.model import LeNet5  # 从main.py文件中导入LeNet5模型类
...
def train_model_process(model, train_loader, val_loader, epochs=10):
    ...
    torch.save(
        model.state_dict(), "./models/le_net5_best_model.pth"
    )  # 保存最终模型参数到文件
...
if __name__ == "__main__":  # 如果当前脚本作为主程序运行
    ...
    result = train_model_process(LeNet5(), traindatam, valdata, 10)
