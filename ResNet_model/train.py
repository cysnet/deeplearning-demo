...
from ResNet_model.model import ResNet  # 从自定义模块导入GoogLeNet
...
def train_val_date_load():  # 定义函数用于加载训练集和验证集
    train_dataset = FashionMNIST(
        ...
        transform=transforms.Compose(
            [
                transforms.Resize(size=224),  # 将图片缩放到224x224
...
def train_model_process(model, train_loader, val_loader, epochs=10):  # 定义训练过程函数
    ...
    torch.save(model.state_dict(), "./models/res_net_best_model.pth")  # 保存模型参数
