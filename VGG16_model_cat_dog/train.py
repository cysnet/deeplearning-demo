...
from VGG16_model_cat_dog.model import VGG16
...
def train_val_date_load():
    ...
    train_loader = DataLoader(
        dataset=train_date, batch_size=batch_size, shuffle=True, num_workers=1
    )

    # 构建验证集的数据加载器，设置批量大小为128，打乱数据，使用8个子进程加载数据
    val_loader = DataLoader(
        dataset=val_data, batch_size=batch_size, shuffle=True, num_workers=1
    )
...
def train_model_process(model, train_loader, val_loader, epochs=10):
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-4)  # 使用Adam优化器，学习率为0.001
    ...
    torch.save(
        model.state_dict(), "./models/cat_dog_vgg16_net_best_model.pth"
    )  # 保存最终模型参数到文件
...
def matplot_acc_loss(train_process):
    ...
    plt.show()  # 显示所有图像
    plt.savefig("./models/cat_dog_vgg16_net_output.png")
