...
from torchvision.datasets import ImageFolder  # 导入FashionMNIST数据集类
...
from LetNet5_model_cat_dog.model import LeNet5  # 从main.py文件中导入LeNet5模型类
...
def train_model_process(model, train_loader, val_loader, epochs=10):
    ...
    torch.save(
        model.state_dict(), "./models/cat_dog_le_net5_best_model.pth"
    )  # 保存最终模型参数到文件
...
def matplot_acc_loss(train_process):
    ...
    plt.show()  # 显示所有图像
    plt.savefig("./models/cat_dog_le_net5_output.png")


if __name__ == "__main__":  # 如果当前脚本作为主程序运行
    traindatam, valdata = train_val_date_load()  # 加载训练集和验证集
    result = train_model_process(LeNet5(2), traindatam, valdata, 30)
