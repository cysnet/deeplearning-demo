...
from GoogLeNet_model.model import GoogLeNet  # 从自定义模块导入GoogLeNet
...
def train_model_process(model, train_loader, val_loader, epochs=10):  # 定义训练过程函数
    ...
    torch.save(model.state_dict(), "./models/google_net_best_model.pth")  # 保存模型参数
...
def matplot_acc_loss(train_process):  # 定义绘图函数
    ...
    plt.show()  # 显示所有图像
    plt.savefig("./models/google_net_output.png")  # 保存图片到指定路径
