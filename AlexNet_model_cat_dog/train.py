...
from AlexNet_model_cat_dog.model import AlexNet  # 从自定义模块导入AlexNet模型
...
def train_model_process(model, train_loader, val_loader, epochs=10):  # 定义训练过程函数
    ...
    torch.save(
        model.state_dict(), "./models/cat_dog_alex_net_best_model.pth"
    )  # 保存模型参数
